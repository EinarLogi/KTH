...
-define(change, 20).
-define(color, {0,0,0}). %% black to begin with
...
start(Id, Module, Rnd, Sleep) ->
	%io:format("PID of worker.erl: ~w~n", [self()]),
    spawn(fun() -> init(Id, Module, Rnd, Sleep) end).

init(Id,  Module, Rnd, Sleep) ->
	%io:format("this is PID of spawned process in worker.erl: ~w~n",[self()]),
    {ok, Cast} = apply(Module, start, [Id]), %% Cast variable has the PID of the leader(note! not the master of the leader!!).
    %io:format("this is cast leader: ~w~n", [Cast]), %% cast should be the leader!
    Color = ?color,
...
join(Id, Cast) ->
    receive
		{view, _} ->
		    %io:format("worker with id: ~w received view message~n",[Id]),
		    Ref = make_ref(),%% returns a unique reference. 
		    %io:format("Worker with id: ~w has cast: ~w~n",[Id,Cast]),
...
state(Id, Ref) ->
    receive
		{state_request, Ref} -> %% we only care about getting the state
	    	receive
				{state, Ref, Color} ->
		    	{ok, Color}
	    	end;
		_Ignore -> %% ignore all other messages that might be delivered 
	    	state(Id, Ref)
    end.
...
init_cont(Id, Rnd, Cast, Color, Sleep) ->
    ...
    Title = "Worker: " ++ integer_to_list(Id), %% integer_to_list Returns a string corresponding to the text representation of Integer, for example: integer_to_list(55) outputs "55".
    %io:format("this PID is sent to GUI: ~w~n", [self()]),
    Gui = gui:start(Title, self()), %% the self() PID is the PID of the spawned process that runs the init function. Not! the same as Cast.
    Gui ! {color, Color},
